pipeline {
    ...
    stages {
        ...
        withCredentials([
            ...
            """
            withCredentials([
                usernamePassword(credentialsId: 'docker-login', passwordVariable: 'DOCKERHUB_PASSWORD', usernameVariable: 'DOCKERHUB_USERNAME')
            ]) {
                sh """
                ...
                """
                sh """
                    sudo docker push kubealon/private-course:poly-bot-${env.BUILD_NUMBER}
                """
            }
        }
    }
